...
# =========================================================
# 1) DATA: Tekstblokken (letterlijk)
# =========================================================
...
BLOCKS = [
    ...
    {"id": 3, "category": "Procedurele gebreken", "section": "Misleiding Contracten",
    ...
    {"id": 4, "category": "Procedurele gebreken", "section": "Onrechtmatige Inzage",
    ...
    {"id": 5, "category": "Procedurele gebreken", "section": "Onrechtmatige Inzage",
    ...
    {"id": 6, "category": "Procedurele gebreken", "section": "Onrechtmatige Inzage",
    ...
    {"id": 7, "category": "Procedurele gebreken", "section": "Integriteit en Vooringenomenheid",
    ...
    {"id": 8, "category": "Procedurele gebreken", "section": "Integriteit en Vooringenomenheid",
    ...
    {"id": 9, "category": "Procedurele gebreken", "section": "Integriteit en Vooringenomenheid",
    ...
    {"id": 10, "category": "Fysieke Leefomgeving", "section": "Maat en Schaal",
    ...
    {"id": 11, "category": "Fysieke Leefomgeving", "section": "Maat en Schaal",
    ...
    {"id": 12, "category": "Fysieke Leefomgeving", "section": "Maat en Schaal",
    ...
    {"id": 13, "category": "Fysieke Leefomgeving", "section": "Milieu en gezondheid",
    ...
    {"id": 14, "category": "Fysieke Leefomgeving", "section": "Milieu en gezondheid",
    ...
    {"id": 15, "category": "Fysieke Leefomgeving", "section": "Milieu en gezondheid",
    ...
    {"id": 16, "category": "Fysieke Leefomgeving", "section": "Milieu en gezondheid",
    ...
    {"id": 17, "category": "Fysieke Leefomgeving", "section": "Waterhuishouding",
    ...
    {"id": 18, "category": "Fysieke Leefomgeving", "section": "Waterhuishouding",
    ...
    {"id": 19, "category": "Fysieke Leefomgeving", "section": "Waterhuishouding",
    ...
    {"id": 20, "category": "Mobiliteit en parkeren", "section": "Verkeersveiligheid",
    ...
    {"id": 21, "category": "Mobiliteit en parkeren", "section": "Verkeersveiligheid",
    ...
    {"id": 22, "category": "Mobiliteit en parkeren", "section": "Verkeersveiligheid",
    ...
    {"id": 23, "category": "Mobiliteit en parkeren", "section": "Parkeerproblematiek",
    ...
    {"id": 24, "category": "Mobiliteit en parkeren", "section": "Parkeerproblematiek",
    ...
    {"id": 25, "category": "Mobiliteit en parkeren", "section": "Parkeerproblematiek",
    ...
    {"id": 26, "category": "Participatie", "section": "Schijnparticipatie",
...
def format_selected_blocks(selected_ids: list[int]) -> str:
    ...
    for bid in selected_ids:
        b = BY_ID[bid]
        out.append(f"## {b['category']} → {b['section']}\n")
        out.append(f"### {b['title']}\n{b['text']}\n")
    return "\n".join(out).strip()

def generate_zienswijze(client: OpenAI, naam: str, adres: str, woonplaats: str, datum_str: str,
                       personal_note: str, selected_ids: list[int], custom_items: list[str]) -> str:
    blocks_payload = format_selected_blocks(selected_ids)

    custom_clean = [normalize_ws(x) for x in (custom_items or []) if normalize_ws(x)]
    custom_block = ""
    if custom_clean:
        custom_block = "## Eigen bezwaren (door indiener)\n" + "\n".join([f"- {x}" for x in custom_clean])
    ...
    with tab:
        ...
        for sec in sections:
            ...
            with st.expander(f"{sec} ({len(ids_filtered)} punten)", expanded=False):
                ...
                with scol1:
                    if st.button("Selecteer sectie", key=f"sel_{cat}_{sec}"):
                        for bid in ids_filtered:
                            st.session_state[f"cb_{bid}"] = True
                        st.rerun()
                    if st.button("Deselecteer sectie", key=f"desel_{cat}_{sec}"):
                        for bid in ids_filtered:
                            st.session_state[f"cb_{bid}"] = False
                        st.rerun()
...
# =========================================================
# 4) Generate button (NO FORM => no 'Missing Submit Button')
# =========================================================

generate_clicked = st.button("🚀 Genereer mijn zienswijze", type="primary", use_container_width=True)

if generate_clicked:
    selected_ids = [b["id"] for b in BLOCKS if st.session_state.get(f"cb_{b['id']}", False)]
    has_custom = any(normalize_ws(x) for x in st.session_state.custom_items)

    if not naam.strip() or not adres.strip():
        st.error("Vul alstublieft uw naam en adres in.")
        st.stop()

    if len(selected_ids) == 0 and not has_custom:
